# selective import for rectangle and circle
from graphics.graphics_3d.cuboid import cuboid_area, cuboid_peri
from graphics.graphics_3d.sphere import sphere_area, sphere_peri
from graphics.rectangle import area, peri
from graphics.circle import area as circle_area, peri as circle_perimeter

# import * for cuboid and sphere (your new area, peri functions)
from graphics.graphics_3d import *

# ----- RECTANGLE -----
print("\n--- Rectangle ---")
l = float(input("Enter rectangle length: "))
b = float(input("Enter rectangle breadth: "))
print("Rectangle area =", area(l, b))
print("Rectangle perimeter =", peri(l, b))


# ----- CIRCLE -----
print("\n--- Circle ---")
r = float(input("Enter circle radius: "))
print("Circle area =", circle_area(r))
print("Circle perimeter =", circle_perimeter(r))


# ----- CUBOID -----
print("\n--- Cuboid ---")
cl = float(input("Enter cuboid length: "))
cb = float(input("Enter cuboid breadth: "))
ch = float(input("Enter cuboid height: "))
print("Cuboid area =", cuboid_area(cl, cb, ch))       # updated
print("Cuboid perimeter =", cuboid_peri(cl, cb, ch))  # updated


# ----- SPHERE -----
print("\n--- Sphere ---")
sr = float(input("Enter sphere radius: "))
print("Sphere area =", sphere_area(sr))       # updated
print("Sphere perimeter =", sphere_peri(sr))  # updated
